...
Automatyczny first-send ma lokalny limiter per środowisko i NIP kontekstu oraz `tries = 1`. Brak pojemności skutkuje zapisaniem joba z opóźnieniem przed finalizacją, utworzeniem submissionu i HTTP. Worker first-send uruchamia się przez `php artisan queue:work ksef_submit --queue=ksef-submit --sleep=1 --tries=1 --timeout=120`; połączenie ma `retry_after = 240 s`. Follow-up pozostaje na `php artisan queue:work database --queue=ksef --sleep=3 --tries=1 --timeout=60`. Nie powstał outbox ani automatyczne wyszukiwanie historycznych Faktur.
