...
Po pełnym sukcesie zapisywane są wyłącznie bezpieczne metadane `remote_status`, `remote_certificate_name`, `remote_valid_from`, `remote_valid_until` i `remote_verified_at`. Zdalne daty są osobnym snapshotem, nie nadpisują ważności X.509 i przed persistence przechodzą przez `KsefInstantStorageNormalizer`. `remote_verified_at` oznacza pełną weryfikację tożsamości, bez dodatkowego TTL w 8B.2A. Jednoznaczna niezgodność tożsamości usuwa poprzedni snapshot zaufania. Przejściowe błędy sieciowe, 429, 5xx, błąd uwierzytelnienia albo niekompletna odpowiedź nie niszczą ostatniej poprawnej weryfikacji. Zmiana lokalnego certyfikatu, klucza lub środowiska podczas requestu jest wykrywana przed zapisem. Etap nie dodaje automatycznej synchronizacji, kolejki, enrollmentu, CSR, zdalnego unieważniania, katalogu certyfikatów MF, wystawiania Offline24 ani KODU II do PDF.
