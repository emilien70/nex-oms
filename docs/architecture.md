...
`KsefConnectionTestService` zawsze wykonuje świeże uwierzytelnienie z zapisanej konfiguracji, niezależnie od `is_active`, a następnie diagnozuje `InvoiceWrite`. `personal/grants` pozostaje źródłem jawnych uprawnień, natomiast `KsefCurrentTokenResolver` uruchamia `GET /tokens` tylko po braku jawnego `InvoiceWrite` i rozpoznaje implicit Owner wyłącznie dla jednego aktywnego Tokena bez kontynuacji, z zakresem `InvoiceWrite` oraz ścisłą zgodnością `Nip` autora, kontekstu i zapisanego NIP-u. Resolver nie parsuje sekretu Tokena ani nie wybiera rekordu heurystycznie; wynik niejednoznaczny pozostaje `null`. Wynik `success`, `warning` albo `error` i niesekretne ostrzeżenie są bieżącym stanem per środowisko, bez tabeli historii. Osobny formularz testu zawiera wyłącznie CSRF; token, NIP i środowisko nie są przyjmowane z browsera. Zmiana Tokena KSeF czyści runtime i wynik testu tego środowiska, a zmiana NIP-u kontekstu robi to dla wszystkich środowisk, zachowując Tokeny KSeF.
...
KSeF.4A.1 dodaje osobny agregat transportowy `KsefInvoiceSubmission` z historią prób dla pary Faktura-środowisko. Rekord zamraża dokładny autorytatywny XML FA(3), schemat, czas generowania, Base64 SHA-256 i rozmiar bajtowy; XML jest szyfrowany at rest przez cast `encrypted`, a FK do Faktury używa `restrictOnDelete`. Numer próby jest wyznaczany pod blokadą Faktury, a stany `preparing`, `session_opened`, `submitted`, `processing`, `accepted` i `uncertain` blokują utworzenie kolejnej aktywnej próby. Historia, która wyszła poza lokalne `preparing`, daje dodatkową kontrolowaną blokadę usuwania Faktury.
...
Granica side effectu POST Faktury jest jawna: błędy przed nią oraz jednoznaczne 4xx prowadzą do `technical_failed`, natomiast timeout, błąd połączenia, 5xx i niekompletna odpowiedź 2xx prowadzą do blokującego `uncertain`. Nie ma automatycznego retry ani persistence materiału potrzebnego do powtórzenia tej samej sesji. Otrzymanie referencji Faktury ustawia tylko `submitted`; błąd close jest przechowywany osobno, a status `accepted` wymaga kodu 200 i prawidłowego numeru KSeF z endpointu statusowego. Nieznany status jest traktowany konserwatywnie.

Transport ma deploymentowy gate domyślnie `false`, jest ograniczony do TEST i nie ma trasy ani UI. KSeF.4A.1 nie dodaje automatycznej wysyłki, kolejki, crona, batch, offline, QR ani UPO. Testy są fake-only i blokują stray HTTP.
