...
Brak TotalFailure issuance, auto-send, odświeżania Latarni przy GET/issuance, zmian schedulera i odblokowania Production. DEMO dopuszcza Offline24, ale nie planned/failure bez źródła Latarni. Nie wykonano live walidacji ani zapisów operatorowych; live E2E jest odłożone do osobnego zadania.
...
Migracja `2026_08_13_087000_add_ksef_offline_correction_financial_evidence.php` dodaje wyłącznie nullable `longText` `ksef_offline_issuances.correction_financial_evidence`, bez indeksu, UPDATE, backfillu lub przepisywania XML; `down()` usuwa kolumnę. Model ma pole w fillable i hidden oraz cast `encrypted:array`; istniejące blokady update/delete obejmują evidence. Każda nowa Korekta Offline24/planned/failure wymaga poprawnego evidence przed CREATE. Zwykła Faktura Offline zachowuje `NULL`. Historyczna Korekta z brakującym lub uszkodzonym evidence jest blokowana, bez rekonstrukcji z XML lub bieżących snapshotów.

Read-only preflight operatora z 05.09.2026: jeden OfflineIssuance zwykłej Faktury, zero historycznych Korekt Offline, zero osieroconych issuance. Migracje `079000–086000`: Ran; `087000`: Pending, nie wykonano jej na bazie operatora. Istniejące rekordy po osobnej migracji otrzymają `NULL`. Evidence pozostaje prywatnym artefaktem OMS: bez serializacji modelu, UI, logów, XML/API payloadu, QR i UPO. Nie zmieniono source/TOCTOU, delivery, terminów, dokładnych bajtów transportu, `offlineMode=true` ani braku `hashOfCorrectedInvoice`. Brak live HTTP, zapisów operatorowych, korekty technicznej, 8C.7 i zmian schedulera.

Status po pełnej regresji: `KSeF.8C.6.1: PASS / CLOSED via 8C.6.2`, `KSeF.8C.6.2: PASS / CLOSED`, `KSeF.8C.6 CODE: PASS / CLOSED`. Unit KSeF: 422 testy / 1412 asercji; Feature KSeF: 927 / 6777; full suite: 2240 / 13800, wszystko PASS. Regresje obejmują zero-delta, niejednoznaczne zaokrąglenie VAT, cross-bucket, walutę obcą/P14W, uszkodzenie XML po aktualizacji outer hash/size/QR, uszkodzone lub brakujące evidence, szyfrowanie, niezmienność, pojedyncze mapowanie oraz istniejące Invoice/Correction workflows. Operator migration `087000`: **PENDING**; zamknięcie operacyjne bazy wymaga osobnego etapu po audycie kodu.
