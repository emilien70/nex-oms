...
Lokalny limiter rezerwuje przed dispatch bezpieczny slot osobno dla pary środowisko + NIP kontekstu. Oficjalne limity PRD dla sesji interaktywnej wynoszą 10/s, 30/min i 120/h dla otwarcia oraz zamknięcia sesji, a DEMO stosuje limity PRD; lokalny profil 8/s, 24/min i 96/h zachowuje margines. Rezerwacja jest zamieniana na opóźnienie joba jeszcze przed first-attempt, dlatego brak pojemności nie tworzy submissionu, nie wykonuje HTTP, nie zużywa frameworkowego retry i nie może uruchomić ponownie invoice POST.
