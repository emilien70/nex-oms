...
Przepływ `review -> download` pokazuje listę i błędy, wymaga osobnego potwierdzenia zestawu oraz HMAC-SHA256 kontekstu, rekordów i własnych dowodów historycznych. To kontrola zmiany danych między ekranem a pobraniem, nie podpis JPK, zapis złożenia ani dowód poprawności podatkowej. Drugi POST ponownie odczytuje raport dokładnie raz; różnica wymaga nowego potwierdzenia. Ręczne oznaczenia są walidowane względem wybranych ID. Przy błędach nie powstaje częściowy eksport. Gotowy XML przechodzi XSD przed odpowiedzią, trafia do prywatnego losowego pliku, jest ponownie sprawdzany i usuwany po wysłaniu/błędzie. Odpowiedź: attachment XML UTF-8, private/no-store/nosniff. Pamięć rośnie z rozmiarem raportu/XML.
