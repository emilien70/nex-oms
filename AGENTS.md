...
Usunąć można wystawioną Fakturę VAT bez Korekt albo aktywną Pro formę, o ile dokument posiada spójny slot, zamówienie i tożsamość numeracji. Pro formy zastąpionej przez istniejącą Fakturę VAT nie wolno usuwać. Operacja wymaga potwierdzenia z numerem dokumentu i oczekiwanej wersji blokady, pozostawia ślad w `order_events`, ale usuwa dokument, jego pozycje, slot i prywatny cache PDF z bieżącego stanu. Usuwanie pojedyncze i zbiorcze jest dostępne na odpowiedniej liście dokumentów, a aktywna Pro forma może zostać usunięta również z karty zamówienia. Pro forma zastąpiona dokładnie przez usuwaną Fakturę zostaje w tej samej transakcji odblokowana, ponownie pokazana w zamówieniu i zachowuje numer oraz bieżący snapshot. Blokady KSeF zostaną rozszerzone po dodaniu pól i procesów tej integracji.
