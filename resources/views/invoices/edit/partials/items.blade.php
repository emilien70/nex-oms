        ...
        <table class="table table-sm invoice-items-table">
            ...
            @foreach($invoice->items as $item)
                <tr>
                    ...
                    <td class="text-center"><button class="invoice-icon-button invoice-edit-item-button" type="button" title="Edytuj pozycję" data-edit-invoice-item data-url="{{ route('invoices.items.update', [$invoice, $item]) }}" data-lock-version="{{ $invoice->lock_version }}" data-item="{{ json_encode(["name"=>$item->name,"description"=>$item->description,"unit_name"=>$item->unit_name,"quantity"=>$item->quantity,"unit_price_gross"=>$item->unit_price_gross,"vat_rate"=>$item->vat_rate,"vat_code"=>$item->vat_code,"position"=>$item->position]) }}"><i class="bi bi-pencil"></i></button></td>
