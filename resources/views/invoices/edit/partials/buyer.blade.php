...
@php($currentBuyer = ['name'=>$order->billing_name,'company_name'=>$order->billing_company_name,'tax_id'=>$order->billing_tax_id,'address_line'=>\App\Support\AddressLineFormatter::formatAddressLine($order->billing_street, $order->billing_building_number, $order->billing_apartment_number),'postal_code'=>$order->billing_postal_code,'city'=>$order->billing_city,'province'=>$order->billing_province,'country_code'=>$order->billing_country_code,'email'=>$order->billing_email,'phone'=>$order->billing_phone])
<section class="invoice-edit-card">
    <header class="invoice-edit-card-header"><h2 class="fs-6 mb-0">Dane nabywcy</h2></header>
    <div class="row g-0">
        <div class="col-lg-6 invoice-edit-card-body">
